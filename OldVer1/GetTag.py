...
def get_html_text(burl, self_header):
    try:
        ...
        encodeFormat = response.apparent_encoding
        if encodeFormat != None and ('window' in encodeFormat or '1254' in encodeFormat or 'Window' in encodeFormat):
            response.encoding = response.encoding
        else:
            response.encoding = encodeFormat
...
def re_get_inf(html, rankArr, i):
    ...
    tags = re.findall(r'<a href="//search.bilibili.com/.*?" target="_blank" class="tag-link">([\s\S]*?)</a>', html)
    tags += re.findall(r'<a href="//www.bilibili.com/.*?" target="_blank" class="tag-link"><span>(.+?)</span></a>', html)
    tagsStr = ''
    tagsArr = []
    for tag in tags:
        print(tag)
